...
export class MarkdownConverter {
  static convertToMarkdown(messages: ParsedMessage[], options: ConversionOptions): ConversionResult {
    try {
      ...
      const filteredMessages = messages.filter(message => !this.isPlaceholderMessage(message))
      ...
      if (options.claudeMode) {
        // Removed system instruction section entirely for now
        markdown += '**Human:**\n\n'
      }
  ...
  private static isPlaceholderText(text: string): boolean {
    ...
    const placeholders = new Set(['INSERT_INPUT_HERE', 'INSERT_YOUR_PROMPT_HERE', 'INSERT_USER_INPUT_HERE'])
